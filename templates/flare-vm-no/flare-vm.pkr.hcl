...
locals {
  template_description = "Flare VM (Windows 11 23H2 64-bit Enterprise) template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => localuser:password"
}

source "proxmox-iso" "flare-vm-no" {
...
build {
  sources = ["source.proxmox-iso.flare-vm-no"]
